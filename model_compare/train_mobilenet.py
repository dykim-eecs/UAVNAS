# train_mobilenet.py (similar changes)
import os
import torch
import argparse
...
def main():
    parser = argparse.ArgumentParser("MobileNetV3 Training")
    ...
    model = models.mobilenet_v3_small(weights=None, num_classes=args.classes)
    model.features[0][0] = torch.nn.Conv2d(2, 16, kernel_size=3, stride=2, padding=1, bias=False)
    ...
    torch.onnx.export(model, dummy_input, 'mobilenet.onnx', opset_version=11)
    print("Exported to mobilenet.onnx")
